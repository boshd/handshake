...
//
//        if channel.participantIds.contains(currentUserID) {
...
//        mutableAttributedString.addAttribute(NSAttributedString.Key.font, value: ThemeManager.currentTheme().secondaryFontVeryBold(with: 16), range: range)
...
//        guard let channelParticipantIds = channel?.participantIds,
//              let currentUserID = Auth.auth().currentUser?.uid
//        else { return }
//           
//        if !channelParticipantIds.contains(currentUserID) {
...
//                } else {
//                    text = "\(globalCurrentUser?.name ?? "") might attend."
//                }
...
//        channelDetailsContainerView.locationView.locationLabel.text = channel?.locationDescription
//        
//        
//        guard let latitude = channel?.latitude.value, let longitude = channel?.longitude.value else { return }
//        if let location = channel?.location {
//            channelDetailsContainerView.locationView.locationLabel.text = parseAddress(selectedItem: MKPlacemark(coordinate: CLLocationCoordinate2D(latitude: location.latitude, longitude: location.longitude)))
