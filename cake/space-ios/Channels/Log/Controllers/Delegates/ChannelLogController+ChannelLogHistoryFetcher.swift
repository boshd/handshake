//
//  ChannelLogController+ChannelLogHistoryFetcher.swift
//  space-ios
//
//  Created by Kareem Arab on 2019-11-10.
//  Copyright © 2021 Kareem Arab. All rights reserved.
//

import UIKit

extension ChannelLogController: ChannelLogHistoryDelegate {
    ...
    func channelLogHistory(updated newMessages: [Message]) {
        ...
        print(channel?.id, channel?.messages.count, groupedMessages.first?.title ?? "", numberOfMessagesInFirstSectionBeforeUpdate, numberOfMessagesInDataSourceBeforeUpdate)
    ...
    fileprivate func batchInsertMS(rowsRange: Int, sectionsRange: Int) {
        UIView.performWithoutAnimation {
            collectionView.performBatchUpdates({
                ...
                stride(from: 0, to: rowsRange, by: 1).forEach { (index) in
    ...
    fileprivate func insertNewSections(_ datesSet: Set<String>) -> Int {
        ...
        for date in datesArray.reversed() {
            guard var messagesInSection = channel?.messages
    ...
    fileprivate func updateFirstSection(_ firstSectionTitle: String,
                                        _ numberOfMessagesInDataSourceBeforeUpdate: Int,
                                        _ numberOfMessagesInFirstSectionBeforeUpdate: Int) -> Int {
        guard var messagesInFirstSectionAfterUpdate = channel?.messages
            .filter("shortConvertedTimestamp == %@", firstSectionTitle)
            .sorted(byKeyPath: "timestamp", ascending: true) else { return 0 }
        
        print("whats going on?")
        channel?.messages.forEach({ (element) in
            print(element.messageUID)
            print(element.convertedTimestamp)
            print(element.shortConvertedTimestamp)
        ...
        if needToLimitFirstSection {
            ...
            print("returning first \(numberOfMessagesInFirstSectionAfterLimiting - numberOfMessagesInFirstSectionBeforeUpdate)")
            return numberOfMessagesInFirstSectionAfterLimiting - numberOfMessagesInFirstSectionBeforeUpdate
        } else {
            let updatedFirstSection = MessageSection(messages: messagesInFirstSectionAfterUpdate, title: firstSectionTitle)
            //configureBubblesTails(for: updatedFirstSection.messages)
            groupedMessages[0] = updatedFirstSection
            print("returning second, \(numberOfMessagesInFirstSectionAfterUpdate), \(numberOfMessagesInFirstSectionBeforeUpdate), \(numberOfMessagesInFirstSectionAfterUpdate - numberOfMessagesInFirstSectionBeforeUpdate)")
            return numberOfMessagesInFirstSectionAfterUpdate - numberOfMessagesInFirstSectionBeforeUpdate
        }
//        guard var messagesInFirstSectionAfterUpdate = channel?.messages.filter("shortConvertedTimestamp == %@", firstSectionTitle).sorted(byKeyPath: "timestamp", ascending: true) else { return 0 }
//
//        let numberOfMessagesInFirstSectionAfterUpdate = messagesInFirstSectionAfterUpdate.count
//        let possibleNumberOfMessagesWithUpdatedSection = (numberOfMessagesInDataSourceBeforeUpdate - numberOfMessagesInFirstSectionBeforeUpdate) + numberOfMessagesInFirstSectionAfterUpdate
//        let maxNumberOfMessagesInChat = numberOfMessagesInDataSourceBeforeUpdate + messagesToLoad
