...
//        tableView.isOpaque = true
    ...
    fileprivate func checkInputsAndReachability() -> Bool {
        ...
        guard let eventName = eventName, !eventName.trimmingCharacters(in: .whitespaces).isEmpty else {
    ...
    func constructHeaderCell(indexPath: IndexPath) -> UITableViewCell {
        ...
        if eventName != nil {
            cell.channelNameField.text = eventName
        }
...
//            "name": eventName?.trimmingCharacters(in: .whitespaces) as AnyObject,
...
//            self?.informationMessageSender.sendInformationMessage(channelID: newChannelReference.documentID, channelName: self?.eventName ?? "", participantIDs: memberIDs.0, text: "New event created. Discuss and share ideas here.", channel: Channel(dictionary: self?.localChannelData))
//
